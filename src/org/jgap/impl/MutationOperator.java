...
import alexdiru.lifesim.jgap.ga.CompositeGeneTreeMutationOperator;
import org.jgap.*;
...
public class MutationOperator
   ...
   */
  protected IUniversalRateCalculator m_mutationRateCalc;

  protected MutationOperatorConfigurable m_config = new
    ...
    for (int i = 0; i < size; i++) {
      ...
      for (int j = 0; j < genes1.length; j++) {
        ...
        if (mutate) {
          ...
          if (this instanceof CompositeGeneTreeMutationOperator) {
             ((CompositeGeneTreeMutationOperator)this).mutateChromosome(copyOfChromosome.getGenes(), generator);
              break;
          }
          else if (genes[j] instanceof ICompositeGene) {
             // if (this instanceof CompositeGeneTreeMutationOperator) {
             //     genes[j] = ((CompositeGeneTreeMutationOperator)this).mutateCompositeGene((ICompositeGene)genes[j],generator);
             // } else {
   ...
   * @since 1.1
   */
  protected void mutateGene(final Gene a_gene, final RandomGenerator a_generator) {
  ...
  public class MutationOperatorConfigurable
